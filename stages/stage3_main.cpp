/*
 * Dethleffs Globebus control panel - STAGE 3 firmware: ADS1115 raw counts + volts
 * Board : Waveshare ESP32-S3-Touch-LCD-4.3B (M-053)
 *
 * Hardware for this stage (CLAUDE.md, Stage 3):
 *   I2C header VOUT (measured 3.31 V) -> ADS1115 VDD, GND -> GND, SDA -> SDA, SCL -> SCL.
 *   ADDR pulled down on the module -> 0x48. Test jumpers: A0 -> GND, A1 -> VDD.
 *   A2, A3 floating (their readings are meaningless noise). No Toptron.
 *
 ...
 *   OS=1 (start) | MUX=1xx (AINx vs GND) | PGA=001 (+-4.096 V FS, 125 uV/LSB) |
 ...
 *
 * Expected: A0 ~ 0 V (a few counts either side), A1 ~ 3.31 V (~26480 counts),
 * matching the multimeter reading of VDD.
 *
 * Screen shows: board profile, uptime, boot I2C scan summary, a table of A0-A3
 * (raw, volts, min/max since reset), read error count, touch readout, RESCAN button.
 */
...
static const char *CH_NOTE[4] = {"(jumper to GND, expect ~0 V)", "(jumper to VDD, expect ~3.31 V)",
                                 "(floating - noise)", "(floating - noise)"};
...
// Single-shot conversion of AINch vs GND. Returns true and the raw count on success.
static bool ads_read_single(uint8_t ch, int16_t *raw)
{
    uint16_t cfg = ADS_OS_START | (uint16_t)((0x4 | ch) << 12) | ADS_PGA_4V096 | ADS_MODE_SINGLE |
...
static void read_all_channels()
{
    ...
    for (uint8_t ch = 0; ch < 4; ch++) {
        int16_t raw;
        Channel &c = g_ch[ch];
        c.valid = ads_read_single(ch, &raw);
        if (!c.valid) continue;
        c.raw = raw;
        c.reads++;
    ...
    if (g_read_cycles % LOG_EVERY == 1) {
        ...
        for (uint8_t ch = 0; ch < 4; ch++) {
            ...
            if (c.valid) {
                Serial.printf("  A%d %6d = %.4f V [%d..%d]", ch, c.raw, c.raw * ADS_LSB_V, c.min, c.max);
            } else {
                Serial.printf("  A%d ERR", ch);
            }
        }
    ...
    for (uint8_t ch = 0; ch < 4; ch++) {
        ...
        if (c.valid) {
            ...
            char line[128];
            snprintf(line, sizeof(line), "A%d   %6d   %7.4f V   min %6d  max %6d   %s", ch, c.raw,
                     c.raw * ADS_LSB_V, c.min, c.max, CH_NOTE[ch]);
            lv_label_set_text(lbl_ch[ch], line);
            lv_obj_set_style_text_color(lbl_ch[ch], ch < 2 ? lv_color_white() : lv_color_hex(0x78909C), 0);
        } else {
            lv_label_set_text_fmt(lbl_ch[ch], "A%d   read error   %s", ch, CH_NOTE[ch]);
...
static void build_ui()
{
    ...
    lv_label_set_text(title, "DETHLEFFS PANEL  -  STAGE 3  ADS1115");
    ...
    lv_label_set_text_fmt(lbl_ads_hdr, "ADS1115 @0x%02X   single-ended, PGA +-4.096 V (125 uV/LSB), 128 SPS, "
    ...
    for (int ch = 0; ch < 4; ch++) {
        ...
        lv_label_set_text_fmt(lbl_ch[ch], "A%d   --", ch);
...
void setup()
{
    ...
    Serial.println("\n=== Dethleffs panel STAGE 3 (ADS1115) ===");
    ...
    Serial.println("Setup done. Boot I2C scan, then ADS1115 A0-A3 every 500 ms (log every 2 s).");
