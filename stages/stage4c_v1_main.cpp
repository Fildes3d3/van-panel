 ...
 *   - On release: prints a one-line summary and shows it on screen. It does NOT know WHICH selector was pressed
 *     (selector sensing comes later) - the user notes that.
 ...
 *   Differential A0-A1 at PGA +-0.512 V (15.625 uV/LSB): with 499 ohm the signal is ~0.2-0.5 V.
 *   (With the final ~86-100 ohm burden, switch to PGA +-0.256 V.)
...
static constexpr Pga PGA_0V512  = {0x0800, 0.512f / 32768.0f};  // PGA = 100
static constexpr Pga PGA_SIGNAL = PGA_0V512;                    // 499 ohm interim burden
...
{
    char line[96];
    snprintf(line, sizeof(line), "PRESSED  %.2f mV   (%.3f mA)   t = %.1f s", mv, mv / BURDEN_OHM,
             t_ms / 1000.0f);
    lvgl_port_lock(-1);
    lv_label_set_text(lbl_live, line);
    lv_obj_set_style_text_color(lbl_live, lv_color_hex(0xFFD54F), 0);
    lvgl_port_unlock();
}
...
static void press_end()
{
    ...
    Serial.printf("[press] #%lu END  %.2f s, %lu samples | first %.2f  @0.2s %s  @0.5s %s  @1s %s  @2s %s  "
                  ...
                  a[1], a[2], a[3], g_press.peak_mv, g_press.last_mv);

    char text[320];
    snprintf(text, sizeof(text),
             "Last press #%lu: %.1f s, %lu samples\n"
             "first %.2f   @0.2 s %s   @0.5 s %s   @1 s %s   @2 s %s mV\n"
             "peak %.2f   last %.2f mV   (burden %.0f ohm, PGA +-0.512 V)",
             (unsigned long)g_press.n, dur / 1000.0f, (unsigned long)g_press.samples, g_press.first_mv, a[0], a[1],
             a[2], a[3], g_press.peak_mv, g_press.last_mv, BURDEN_OHM);
    lvgl_port_lock(-1);
...
static void build_ui()
{
    ...
    lbl_live = make_label(scr, &lv_font_montserrat_30, lv_color_hex(0x90A4AE));
    lv_label_set_text(lbl_live, "No selector pressed");
    lv_obj_align(lbl_live, LV_ALIGN_TOP_LEFT, 40, 172);

    lbl_summary = make_label(scr, &lv_font_montserrat_20, lv_color_white());
    ...
    lv_obj_align(lbl_summary, LV_ALIGN_TOP_LEFT, 40, 226);

    lbl_diag = make_label(scr, &lv_font_montserrat_16, grey);
    lv_label_set_text(lbl_diag, "Idle diagnostics: --");
    lv_obj_align(lbl_diag, LV_ALIGN_TOP_LEFT, 40, 340);

    lv_obj_t *btn = lv_btn_create(scr);
    lv_obj_set_size(btn, 200, 64);
    lv_obj_align(btn, LV_ALIGN_BOTTOM_RIGHT, -50, -30);
