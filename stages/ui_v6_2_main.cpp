...
static constexpr time_t TIME_VALID_AFTER = 1704067200;   // 2024-01-01: anything earlier = clock not set
...
static void run_diagnostics()
{
    ...
    snprintf(line, sizeof(line),
             ...
             "Uptime %02lu:%02lu:%02lu   presses %lu   free heap %lu B (internal %lu B)   built " __DATE__ " " __TIME__
...
static void build_settings_screen()
{
    ...
    lv_obj_t *tabs[5];
    const char *names[5] = {"Clock", "Wi-Fi", "Bluetooth", "Weather", "Diagnostics"};
    for (int i = 0; i < 5; i++) {
...
void loop()
{
    ...
    if (now - last_poll_ms >= IDLE_POLL_MS) {
        last_poll_ms = now;
        float v;
        if (ads_read_volts(MUX_DIFF_01, PGA_SIGNAL, &v) && v * 1000.0f >= PRESS_THRESHOLD_MV) {
